...
public class GDChangesButton extends StyledButton {
	...
	public GDChangesButton(Chrome.Type type, String label) {
		super(type, label);
	...
	public void update() {
		...
		if (!updateShown && Updates.updateAvailable()) {
			...
			text(Messages.get(GDChangesButton.class, "downloadsuccessyeah"));
		}
		...
		if(downloadSuccess){
			...
			textColor(ColorMath.interpolate(0xFFFFFF, 0xFFB501, 0.5f + (float) Math.sin(Game.timeTotal * 5) / 2f));
		} else if(updateShown) {
			textColor(ColorMath.interpolate(0xFFFFFF, 0xFFB501, 0.5f + (float) Math.sin(Game.timeTotal * 5) / 2f));
		}
	}
	...
	protected void onClick() {
		if (Updates.updateAvailable()) {
			...
			GirlsFrontlinePixelDungeon.scene().addToFront(new WndUpdate(update));
		} else {
	...
	public class LoadingSprite extends MobSprite {

		public LoadingSprite() {
			super();

			texture( Assets.Interfaces.LOADING );

			TextureFilm frames = new TextureFilm( texture, 7, 7 );

			idle = new Animation( 11, true );
			idle.frames( frames, 3,2,1,0 );
	...
	public class WndUpdate extends Window {
		...
		private PlatformSupport.UpdateCallback listener = new PlatformSupport.UpdateCallback() {
			...
			public void onStart(String url) {
				updateProgress = Messages.get(GDChangesButton.class,"downloading");
				downloadStart = true;
			}
			...
			public void onFinish(File file) {
				...
				downloadStart = false;
				GDChangesButton.file = file;
				updateProgress = Messages.get(GDChangesButton.class,"downloadsuccess");
				GLog.p(Messages.get(GDChangesButton.class, "downloadsuccessling"));
			}

			@Override
			public void onError(Exception e) {
				updateProgress = Messages.get(GDChangesButton.class,"downloadingfailed");
		...
		public WndUpdate(AvailableUpdateData update) {
			...
			float pos = 0;
			IconTitle tfTitle = new IconTitle(new LoadingSprite(), update.versionName == null ? Messages.get(GDChangesButton.class, "title") : Messages.get(GDChangesButton.class, "versioned_title", update.versionName));
			tfTitle.setRect(0, pos, width, 0);
			add(tfTitle);

			pos = tfTitle.bottom() + 2 * MARGIN;

			layoutBody(pos, update.desc == null ? Messages.get(GDChangesButton.class, "desc") : update.desc, update);
		}
		...
		protected void layoutBody(float pos, String message, AvailableUpdateData update) {
			...
			RedButton btn = new RedButton(DeviceCompat.isDesktop() ? Messages.get(GDChangesButton.class,"downloadpc") : Messages.get(GDChangesButton.class,"download1")) {
				@Override
				public void update() {
					if(downloadSuccess) {
				...
				protected void onClick() {
					if(Objects.equals(update.URL1, "null") && !downloadSuccess){
						GirlsFrontlinePixelDungeon.scene().add( new WndError( Messages.get(GDChangesButton.class, "null") ) );
					} else if(DeviceCompat.isDesktop()){
						GirlsFrontlinePixelDungeon.platform.openURI( update.URL3 );
					} else if (!downloadSuccess) {
			...
			RedButton btn3 = new RedButton(Messages.get(GDChangesButton.class,"download3")) {
				@Override
				protected void onClick() {
					GirlsFrontlinePixelDungeon.switchNoFade(ChangesScene.class);
				}
