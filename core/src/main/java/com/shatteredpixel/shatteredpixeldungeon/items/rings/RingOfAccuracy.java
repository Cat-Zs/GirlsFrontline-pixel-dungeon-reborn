...
public class RingOfAccuracy extends Ring {
	...
	public String statsInfo() {
		if (isIdentified()){
			String info = Messages.get(this, "stats", new DecimalFormat("#.##").format(100f * (Math.pow(1.3f, soloBuffedBonus()) - 1f)));
            if (isEquipped(Dungeon.hero) && soloBuffedBonus() != combinedBuffedBonus(Dungeon.hero)) {
                info = info + "\n\n" + Messages.get(this, "combined_stats", Messages.decimalFormat("#.##", 100.0F * (Math.pow(1.3F, combinedBuffedBonus(Dungeon.hero)) - 1.0F)));
            }
            return info;
		} else {
